...
_PROMPT_TEMPLATE = """
用户会提出一个地名，你的目标是拆分出用户给出地名中的市，区 并按照我提供的工具回答。
例如 如果用户提出的问题是: 上海市黄浦区
则 提取的市和区是: 上海 黄浦
如果用户提出的问题是: 上海市
则 提取的市和区是: 上海 None
如果用户提出的问题是: 上海浦东未来1小时天气情况？
则 提取的市和区是: 上海 浦东
如果用户提出的问题是: 上海今天天气情况？
则 提取的市和区是: 上海 None
...
def weather(query):
    ...
    logger.info(f"切分的结果是: {location}  区  {adm} 市")
    key = KEY
    if key == "":
...
class WhetherSchema(BaseModel):
    location: str = Field(description="应该是一个市级行政区的名称,例如：嘉兴市，或者上海市，如果你不知道某个地方属于哪个市，可以直接输入嘉兴市")
